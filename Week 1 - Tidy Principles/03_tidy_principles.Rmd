...
fish_encounters %>%
  pivot_wider(
    names_from = station, 
    values_from = seen,
    values_fill = list(seen = 0) ## since seen = 0 is missing from data, you want to fill this in (explicit missing data)
  )
```
...
fill(stock, year) ## fills in missing repeated values. 
```
...
starwars %>%
  filter(hair_color %in% c("black", "brown")) ## contains black or brown
```
...
nested <- gapminder %>%
  ...
  nest()
head(nested)
nested$data %>% pluck(1) ## take the first dataframe
```
...
`map()` for one argument and `map2()` for two arguments; `map_if()` if you want to apply to certain types of variables. 
